<?php
abstract class CasinosListController extends Controller {
	public function run() {
		
$this->response->setAttribute("parents", array (
  0 => 'random#9',
));
$this->response->setAttribute("country", 'random#5');
$this->response->setAttribute("casinos", array (
  0 => 
  array (
    'id' => 8,
    'name' => 'random#9',
    'rating' => 8,
    'is_accepted' => 1,
    'softwares' => 
    array (
      0 => 'random#3',
      1 => 'random#4',
      2 => 'random#7',
      3 => 'random#4',
      4 => 'random#5',
      5 => 'random#10',
      6 => 'random#4',
      7 => 'random#8',
      8 => 'random#7',
      9 => 'random#2',
    ),
    'bonus_first_deposit' => 
    array (
      'amount' => 'random#4',
      'min_deposit' => 'random#8',
      'wagering' => 'random#6',
      'games_allowed' => 
      array (
        0 => 'random#3',
        1 => 'random#7',
        2 => 'random#5',
        3 => 'random#9',
        4 => 'random#3',
        5 => 'random#5',
        6 => 'random#2',
        7 => 'random#5',
        8 => 'random#3',
        9 => 'random#8',
      ),
      'code' => 'random#6',
    ),
    'bonus_free' => 
    array (
      'amount' => 'random#3',
      'min_deposit' => 'random#7',
      'wagering' => 'random#10',
      'games_allowed' => 
      array (
        0 => 'random#10',
        1 => 'random#5',
        2 => 'random#7',
        3 => 'random#9',
        4 => 'random#8',
        5 => 'random#1',
        6 => 'random#5',
        7 => 'random#1',
        8 => 'random#5',
        9 => 'random#5',
      ),
      'code' => 'random#4',
    ),
    'languages' => 
    array (
      0 => 'random#2',
      1 => 'random#1',
      2 => 'random#6',
      3 => 'random#6',
      4 => 'random#9',
      5 => 'random#2',
      6 => 'random#9',
      7 => 'random#6',
      8 => 'random#7',
      9 => 'random#7',
    ),
    'currencies' => 
    array (
      0 => 'random#8',
      1 => 'random#1',
      2 => 'random#8',
      3 => 'random#3',
      4 => 'random#4',
      5 => 'random#6',
      6 => 'random#9',
      7 => 'random#6',
      8 => 'random#2',
      9 => 'random#8',
    ),
    'is_live_dealer' => 1,
    'date_established' => '2018-01-17 15:37:12',
    'emails' => 
    array (
      0 => 'random#7',
      1 => 'random#5',
      2 => 'random#4',
      3 => 'random#4',
      4 => 'random#6',
      5 => 'random#9',
      6 => 'random#4',
      7 => 'random#10',
      8 => 'random#4',
      9 => 'random#8',
    ),
    'phones' => 
    array (
      0 => 'random#2',
      1 => 'random#4',
      2 => 'random#4',
      3 => 'random#8',
      4 => 'random#3',
      5 => 'random#5',
      6 => 'random#6',
      7 => 'random#9',
      8 => 'random#1',
      9 => 'random#2',
    ),
    'is_live_chat' => 1,
    'licenses' => 
    array (
      0 => 'random#2',
      1 => 'random#10',
      2 => 'random#9',
      3 => 'random#5',
      4 => 'random#6',
      5 => 'random#7',
      6 => 'random#1',
      7 => 'random#7',
      8 => 'random#5',
      9 => 'random#6',
    ),
    'certifiers' => 
    array (
      0 => 'random#4',
      1 => 'random#10',
      2 => 'random#10',
      3 => 'random#7',
      4 => 'random#5',
      5 => 'random#8',
      6 => 'random#1',
      7 => 'random#5',
      8 => 'random#1',
      9 => 'random#8',
    ),
    'affiliate_program' => 'random#7',
    'withdrawal_minimum' => 'random#5',
    'withdrawal_limits' => 
    array (
      0 => 'random#1',
      1 => 'random#4',
      2 => 'random#8',
      3 => 'random#6',
      4 => 'random#9',
      5 => 'random#6',
      6 => 'random#7',
      7 => 'random#1',
      8 => 'random#2',
      9 => 'random#8',
    ),
    'withdrawal_timeframes' => 
    array (
      0 => 'random#1',
      1 => 'random#1',
      2 => 'random#3',
      3 => 'random#6',
      4 => 'random#8',
      5 => 'random#3',
      6 => 'random#3',
      7 => 'random#3',
      8 => 'random#9',
      9 => 'random#6',
    ),
    'deposit_methods' => 
    array (
      0 => 'random#2',
      1 => 'random#8',
      2 => 'random#3',
      3 => 'random#6',
      4 => 'random#6',
      5 => 'random#3',
      6 => 'random#10',
      7 => 'random#7',
      8 => 'random#10',
      9 => 'random#6',
    ),
    'withdraw_methods' => 
    array (
      0 => 'random#2',
      1 => 'random#1',
      2 => 'random#10',
      3 => 'random#10',
      4 => 'random#7',
      5 => 'random#8',
      6 => 'random#6',
      7 => 'random#3',
      8 => 'random#9',
      9 => 'random#8',
    ),
  ),
));
$this->response->setAttribute("total_casinos", 1);
	}
}
