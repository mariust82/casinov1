...
class CasinoReviews
{
    ...
    public function getAllTotal($casinoID) {
        return SQL("SELECT COUNT(id) AS nr FROM casinos__reviews WHERE casino_id = :casino_id AND parent_id = 0 AND (status = ".ReviewStatuses::APPROVED ." OR t1.status = ".ReviewStatuses::NONE.")",array(":casino_id"=>$casinoID))->toValue();
    }
